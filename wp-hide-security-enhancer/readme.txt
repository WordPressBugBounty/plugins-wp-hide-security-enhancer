...
* Fix: 2FA feature, if the option require_setup is active, show the other available methods, so the user can still continue with the login, even if not able to set-up all options.

= 2.8.5 =
* Add a Settings link to the plugin within the Plugins area to make it easier for users to get started with the plugin.
* Fix the 2FA flow when the “Enforce User to Configure 2FA” option is enabled. Currently, if some 2FA methods are not configured, the “no-setup required” options are not displayed, forcing users to complete setup for all methods. Ensure that users can see and select available 2FA options that do not require prior setup, even if other methods remain unconfigured. 
* WordPress 7.0 compatibility check
* WordPress 7.0 tag update

= 2.8.3 =
* Better explain the Scan functionality built on insights generated by our proprietary AI-engineered with advanced models, the scan checks your site against 99.9% of identified security issues and vulnerabilities.
